#ifndef CPPAD_CG_ARRAY_COMPRESSOR_INCLUDED
#define CPPAD_CG_ARRAY_COMPRESSOR_INCLUDED
/* --------------------------------------------------------------------------
...
public:
  /**
   * Creates an ArrayIdCompressor
   * @param maxArraySize The likely number of elements in the temporary array
   */
  inline ArrayIdCompressor( CodeHandlerVector<Base, size_t> &varId, size_t maxArraySize )
  ...
  inline size_t reserveArraySpace( const OperationNode<Base> &newArray )
  {
    ...
    for ( size_t i = 0; i < args.size(); i++ ) {
      ...
      if ( argOp != nullptr && argOp->getOperationType() == CGOpCode::ArrayElement ) {
        ...
        CPPADCG_ASSERT_UNKNOWN( _varId[otherArray] >
                                0 ); // make sure it had already been assigned space
    ...
    for ( size_t i = 0; i < arraySize; i++ ) { _tmpArrayValues[bestStart + i] = &args[i]; }
